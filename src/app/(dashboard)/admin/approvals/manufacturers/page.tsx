...
import { useAdminManufacturerRegistrations, useManufacturerOptions } from "@/hooks/use-admin-manufacturer-registrations";
...
export default function ManufacturerApprovalsPage() {
  ...
  const { data, isLoading, isError, refetch, isRefetching } = useAdminManufacturerRegistrations({
    status,
  ...
  const { data: manufacturerOptions, isLoading: isLoadingOptions } = useManufacturerOptions();
    ...
    <div className="flex flex-col gap-6">
        ...
        <CardHeader className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-2">
            <CardTitle className="flex items-center gap-2 text-xl font-semibold">
              <ShieldCheck className="size-5" /> Manufacturer Approvals
            </CardTitle>
            <CardDescription>Review pending manufacturer signups and link them to existing records.</CardDescription>
          </div>
      ...
      <Dialog open={isActionDialogOpen} onOpenChange={setIsActionDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {actionType === "approve" ? "Approve Manufacturer Registration" : "Reject Manufacturer Registration"}
            </DialogTitle>
            <DialogDescription>
              {actionType === "approve"
                ? "Link the signup to an existing manufacturer record."
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className="w-full">
                    <SelectValue placeholder={isLoadingOptions ? "Loading manufacturers..." : "Choose manufacturer"} />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <DialogFooter className="gap-2">
            ...
            {actionType === "approve" ? (
              <Button onClick={handleApprove} disabled={!selectedManufacturerId}>
                <Check className="mr-2 size-4" /> Approve
              </Button>
            ) : (
